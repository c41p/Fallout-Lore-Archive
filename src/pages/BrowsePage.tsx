...
import { searchReference } from "../reference/provider";
...
export function BrowsePage() {
  ...
  const [referenceResults, setReferenceResults] = useState<ReferenceSearchResult[]>([]); const [referenceError, setReferenceError] = useState(""); const [referenceLoading, setReferenceLoading] = useState(false);
  ...
  useEffect(() => { if (!query || type !== "all" || game) { setReferenceResults([]); setReferenceError(""); return; } let active = true; setReferenceLoading(true); setReferenceError(""); searchReference(query).then((values) => active && setReferenceResults(values)).catch((reason: Error) => active && setReferenceError(reason.message)).finally(() => active && setReferenceLoading(false)); return () => { active = false; }; }, [query, type, game]);
  ...
  return <div className="page"><header className="page-header"><span className="kicker">{game ? "FILTERED WORK INDEX" : "GLOBAL INDEX"}</span><h1>Search & browse</h1><p>Search canonical names, aliases, summaries, article sections and tags.{game && " Results are limited to the selected game."}</p></header>
    ...
    {query && type === "all" && !game && <section className="reference-results"><div className="section-heading"><div><span className="eyebrow"><BookOpen size={13} /> REMOTE REFERENCE SEARCH</span><h2>Nukapedia articles</h2></div><span className="count-label">{referenceLoading ? "Searching provider…" : `${referenceResults.length} matches`}</span></div>{referenceError && <p className="reference-search-error">Provider search is temporarily unavailable: {referenceError}</p>}<div>{referenceResults.map((result) => <Link key={result.pageId} to={result.entityId ? `/entity/${result.entityId}` : `/reference/${result.providerId}?pageId=${result.pageId}`}><div><strong>{result.title}</strong><p>{result.snippet || "Open the integrated reference article."}</p><span>{result.entityId ? "Connected Archive record" : "Provider-only article"}</span></div><ArrowRight /></Link>)}</div></section>}
  </div>;
}
