import { ArrowLeft, BookOpen, CalendarDays, ExternalLink, FileText, GitBranch, List, MapPin, Quote, ShieldAlert, Tag } from "lucide-react";
...
export function EntityPage() {
  ...
  const provenance = [...evidence.values()].sort((left, right) => Number(right.item.sourceClass === "primary") - Number(left.item.sourceClass === "primary") || Number(right.link.directness === "explicit") - Number(left.link.directness === "explicit") || left.work.title.localeCompare(right.work.title));
  return <div className="page entity-page">
    ...
    <nav className="record-nav" aria-label="Record sections">{[["reference-article", "Article"], ["structured-history", "Timeline & facts"], ["relationships", "Connections"], ["geography", "Map"]].map(([target, label]) => <button type="button" key={target} onClick={() => document.getElementById(target)?.scrollIntoView()}>{label}</button>)}</nav>
    ...
    {(entity.articleMode === "reference" || entity.articleMode === "hybrid") && <ReferenceArticle entityId={entity.id} />}
    {detail.articleSections.length > 0 && <div className="article-layout">
      ...
      <article className="lore-article"><div className="section-heading"><div><span className="eyebrow">ARCHIVE SYNTHESIS</span><h2>Local research and context</h2></div><span className="count-label"><FileText size={15} /> {detail.articleSections.length} sections</span></div>
    ...
    {provenance.length > 0 && <section className="content-panel"><span className="eyebrow">PROVENANCE</span><h2>Sources used on this record</h2><p className="section-intro">Source metadata and locators are retained without redistributing extensive copyrighted text. Primary and explicit sources are shown first.</p><EvidenceList evidence={provenance.slice(0, 8)} />{provenance.length > 8 && <details className="section-sources provenance-more"><summary>Show {provenance.length - 8} additional source{provenance.length - 8 === 1 ? "" : "s"}</summary><EvidenceList evidence={provenance.slice(8)} /></details>}</section>}
