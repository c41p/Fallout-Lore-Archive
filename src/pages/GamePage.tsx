import { BookOpen, CalendarRange, ExternalLink, Map, Search } from "lucide-react";
...
export function GamePage() {
  ...
  return <div className="page game-page"><header className="game-hero"><span className="kicker">GAME ARCHIVE · {work.releaseDate?.slice(0, 4)}</span><h1>{work.title}</h1><p>{work.description}</p><div className="game-actions"><Link to={`/browse?game=${encodeURIComponent(work.id)}`}><Search /> Browse {entities.length} records</Link><Link to={`/timeline?game=${encodeURIComponent(work.id)}`}><CalendarRange /> Game timeline</Link><Link to={`/map?game=${encodeURIComponent(work.id)}`}><Map /> Game map</Link></div></header>
    ...
    <section className="content-panel source-browser"><span className="eyebrow"><BookOpen size={13} /> SOURCE INDEX</span><h2>{sourceItems.length} source locators</h2><p className="section-intro">Released-game evidence, official supplementary material and clearly labelled research references are indexed as metadata. The archive paraphrases rather than redistributing extensive copyrighted text.</p><div>{sourceItems.map((item) => <article key={item.id}><strong>{item.title}</strong><span>{item.sourceClass.replaceAll("_", " ")} · {item.sourceType.replaceAll("_", " ")} · {item.locator}</span>{item.context && <p>{item.context}</p>}{item.url && <button className="source-link" type="button" onClick={() => void openExternalSource(item.url!)}>Open source <ExternalLink size={12} /></button>}</article>)}</div></section>
  </div>;
}
